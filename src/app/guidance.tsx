  ...
  return (
    <Chromed title="Guidance">
      ...
      <View style={{ padding: t.space.lg }}>
        <Text style={s.callout}>No stored protocol for this yet. See a health worker.</Text>
      </View>
    </Chromed>
  );
}
 ...
 *  protocol row has no structured `steps` yet. */
function bodyFallbackSteps(body: string): ApiProtocolStep[] {
  return body
    .split('\n')
    .filter(Boolean)
    .map((line) => ({ label: '', head: line, why: '', tier: 'normal' as const }));
}
...
const ObservedProtocolLookup = withObservables(['slug'], ({ slug }: { slug: string }) => ({
  protocols: database.get<ProtocolModel>('protocols').query(Q.where('slug', slug)).observe(),
}))(ProtocolLookup);
