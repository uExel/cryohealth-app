...
Session: task5-ship  Model: claude-sonnet-5  Branch: main  Goal: #1  Task: #5

## State
Task #5 escalated (see `docs/ai/sessions/2026-09-22-task5-escalated-handoff.md`), then
**human decision made: ship as-is.** Production's 11 real protocols will show "not
available yet" in public mode and body-line CHW fallback until real `steps` content is
authored by a human — that's the designed, safe behavior. Stale `CLAUDE.md` line fixed
(`cfb65b8`). About to push to `origin/main` — note this repo has no auto-deploy pipeline;
"deployment" for the mobile app still means a separate EAS build/submit to TestFlight,
...
## Next action
Push `origin/main`. Separately (not blocking this push): someone still needs to
